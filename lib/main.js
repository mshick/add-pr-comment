...
const run = async () => {
    try {
        ...
        const output = await promises_1.default.readFile(process.env['GITHUB_OUTPUT'], 'utf8');
        const state = await promises_1.default.readFile(process.env['GITHUB_STATE'], 'utf8');
        const stepSummary = await promises_1.default.readFile(process.env['GITHUB_STEP_SUMMARY'], 'utf8');
